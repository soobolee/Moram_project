...
@WebServlet("/MoramRegist.do")
@MultipartConfig(
		fileSizeThreshold = 1024 * 1024 * 10,
		maxFileSize = 1024 * 1024 * 30, maxRequestSize = 1024 * 1024 * 50
		
)
public class MoramRegist extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MoramMemberVO vo2 = new MoramMemberVO();
		
		
		IMoramService service = MoramServiceImpl.getInstance();
		String mrNo = service.insertMoram(vo);
		String mem_id = (String)session.getAttribute("sessionId");
		vo.setMr_no(mrNo);
		vo.setMoramjigi(mem_id);
		
		vo2.setMem_id(mem_id);
		vo2.setMr_no(mrNo);
		System.out.println(mrNo);
		
		service.insertMoramjigi(vo2);
		// upload한 파일이 저장될 폴더 설정
		String uploadPath = "D:\\A_TeachingMaterial\\4.MiddleProject\\workspace\\Moram\\WebContent\\images\\moram\\";
	...
	//	uploadPath = uploadPath + folderPath;
		...
		for(Part part : request.getParts()) {
			...
			if(!"".equals(fileName)) {
				...
				fileName = mrNo+fileName.substring(fileName.indexOf("."), fileName.length());
				
				vo.setMr_image(fileName);
				service.moramImgUpdate(vo);
				try {
